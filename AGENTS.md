...
validation and the complete hygiene pentad are green at the **exact current head**,
an exact-head CodeRabbit or Codex review is green, and the change has been tried
and evaluated through a real user path. Immediately before merging, re-read the
head, author, draft state, review surfaces, and mergeability; merge only a CLEAN
promoted pull request with bare `gh pr merge <number> --squash`. Never use
`--auto` on an agent-owned pull request.
